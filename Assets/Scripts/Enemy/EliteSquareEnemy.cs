...
public class EliteSquareEnemy : BasicSquareEnemy
{
    protected override void onStartMod(){
        deathTime = 0;
        moveDirection = Vector2.zero;
        spriteRenderer = GetComponent<SpriteRenderer>();
        StartCoroutine(movement(Random.Range(2,3),true));
        StartCoroutine(shoot(Random.Range(3,8),true));
    }

    new protected IEnumerator shoot(float time,bool again){
        ...
        bul.endVelocity = shootVector*7.5f;
        ...
        if(again){
            StartCoroutine(shoot(Random.Range(2,6),again));
        }
    }
    ...
    protected override IEnumerator movement(float time,bool again)
    {
        ...
        if(again){
            StartCoroutine(movement(Random.Range(2,4f),again));
        }
    }
}
